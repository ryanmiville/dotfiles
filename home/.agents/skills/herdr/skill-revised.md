---
name: herdr
description: "Controls Herdr, a terminal multiplexer for coding agents. Use when the user explicitly mentions Herdr, asks to inspect or control its panes, tabs, workspaces, terminals, commands, or agent communication, or asks to spin up or create a sub-agent. Do not trigger for implicit delegation or parallel-work opportunities. Requires HERDR_ENV=1."
---
...
When the user asks to spin up or create sub-agents, create **pi sub-agents** by launching `pi`. Never put them in the caller's current tab. Unless the user explicitly requests another topology, create a new tab in the current workspace, using the caller's current working directory and `--no-focus`.

Treat all sub-agents requested in one prompt as one batch:

- Create one new tab for the batch.
- Use that tab's initial pane for the first sub-agent and split additional panes in that tab for the rest.
- Give each pane a task-specific label, launch `pi`, wait for `idle`, then submit its task as described below.
- Keep related or adversarial reviewers together in the batch tab. Distinct tasks may also share it; do not create separate tabs merely because their tasks differ.

A later user prompt requesting another sub-agent starts a new batch and therefore a new tab, even if earlier sub-agents have finished. Explicit user instructions to use particular tabs, workspaces, or another organization override these defaults.

Discover `herdr tab` syntax first, then create the batch tab explicitly in the current workspace:

```bash
herdr tab create --workspace "$HERDR_WORKSPACE_ID" --cwd "$PWD" --label "sub-agents" --no-focus
```

Read the returned tab and initial pane IDs from JSON. Create any additional panes by explicitly splitting a pane in that new tab, never the caller pane:

```bash
herdr pane split --pane <batch-pane-id> --direction right --no-focus
```

Follow the interactive startup and coordination procedure below for every sub-agent, always using `pi` as its executable.

## Start agents interactively

For ordinary agent requests that are not sub-agent requests, default to a sibling pane in the current tab and current working directory. Do not create a workspace, tab, worktree, or different cwd unless the user explicitly requests that topology or location. Sub-agent requests follow the new-tab rules above.

When creating that ordinary sibling pane, honor a direction requested by the user. Otherwise inspect the caller pane's current rectangle:
...
Replace `right` with `down` when the layout calls for it. For sub-agents, skip this caller-pane split and use the panes prepared in the batch tab above.

Read `result.pane.pane_id` from the relevant create or split JSON response. Give the pane a useful label, then start the requested agent by running only its normal executable so its interactive TUI opens:
