...
- `nav.js`: Shared navigation script + page manifest. Scaffold from [nav-template.js](./nav-template.js) when creating the first page. See [NAV-FORMAT.md](./NAV-FORMAT.md).
- `nav.css`: Shared navigation styles. Scaffold from [nav-template.css](./nav-template.css) alongside `nav.js`.
...
All lessons and reference pages share a sidebar nav so the user can move between pages without leaving the browser. See [NAV-FORMAT.md](./NAV-FORMAT.md) for full details.

**When creating any HTML page:**

1. If `nav.js` does not exist yet, scaffold it and `nav.css` from the templates in this skill directory. Set the `topic` field.
2. Add `<link rel="stylesheet" href="../nav.css">` and `<script src="../nav.js" defer></script>` to the page's `<head>`.
3. Add an entry to the `TEACH_NAV_MANIFEST` in `nav.js` for the new page.

The nav uses a fixed neutral dark palette and system fonts — do **not** restyle it per lesson. It is the visual constant across all pages; lesson content provides the variety.
