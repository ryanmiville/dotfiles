...
function renderTreeDetail(state: TreeDetailState, width: number): string[] {
  ...
  const heading =
    state.title ||
    `${state.entry.role} · ${relativeTime(state.entry.timestamp)}`;
  ...
  lines.push(
    treeBorder(`╭${"─".repeat(left)}`) +
      styledTitle +
      treeBorder(`${"─".repeat(right)}╮`),
  );
  lines.push(treeDetailRow(width, ""));

  if (!state.lines || state.width !== width) {
    state.lines = new Markdown(
      state.entry.markdown,
      0,
      0,
      getMarkdownTheme(),
  ...
  const slice = detailLines.slice(
    state.scroll,
    state.scroll + TREE_DETAIL_BODY_HEIGHT,
  );
  for (const md of slice) lines.push(treeDetailRow(width, md));
  for (let i = slice.length; i < TREE_DETAIL_BODY_HEIGHT; i++)
    lines.push(treeDetailRow(width, ""));
  ...
  if (total > TREE_DETAIL_BODY_HEIGHT) {
    ...
    lines.push(
      treeDetailRow(width, `${up}${down} ${first}–${last}/${total} lines`),
    );
  } else {
    lines.push(treeDetailRow(width, ""));
  }
  ...
  lines.push(
    treeDetailRow(
      width,
      `${ITALIC("↑↓/PgUp/PgDn")} scroll  ${ITALIC("Home/End")} ends  ${ITALIC("esc")} tree`,
    ),
  );
  lines.push(treeBorder(`╰${"─".repeat(Math.max(0, width - 2))}╯`));
  return lines;
...
function handleTreeDetailInput(state: TreeDetailState, data: string): boolean {
  ...
  if (matchesKey(data, Key.up) || data === "k")
    ...
    state.scroll = clampTreeDetailScroll(state, state.scroll + 1);
  else if (matchesKey(data, Key.pageDown) || matchesKey(data, Key.ctrl("d")))
    state.scroll = clampTreeDetailScroll(
      state,
      state.scroll + TREE_DETAIL_BODY_HEIGHT,
    );
  else if (matchesKey(data, Key.pageUp) || matchesKey(data, Key.ctrl("u")))
    state.scroll = Math.max(0, state.scroll - TREE_DETAIL_BODY_HEIGHT);
  else if (matchesKey(data, Key.home)) state.scroll = 0;
  else if (matchesKey(data, Key.end) || data === "G")
    state.scroll = clampTreeDetailScroll(state, Number.MAX_SAFE_INTEGER);
  else return false;
  return true;
}
...
function patchTreeHelpText(selector: TreeSelectorInternals): void {
  ...
  const help = selector.children?.find((child) => {
    ...
    return (
      text.includes("session tree") === false &&
      text.includes("label") &&
...
function installPinnedFilter(list: TreeListInternals): void {
  if (list.__pinnedFilterPatched || typeof list.applyFilter !== "function")
    return;
...
function installTreePinToggle(pi: ExtensionAPI): void {
  const proto =
    TreeSelectorComponent.prototype as unknown as TreeSelectorInternals;
  if (proto.__pinTogglePatched) return;
  const originalInput = proto.handleInput;
  const originalRender = proto.render;
  if (
    typeof originalInput !== "function" ||
    typeof originalRender !== "function"
  )
    return; // host shape changed — bail safely
  proto.handleInput = function (
    this: TreeSelectorInternals,
    data: string,
  ): void {
  ...
  proto.render = function (
    ...
    return this.__messageDetail
      ? renderTreeDetail(this.__messageDetail, width)
